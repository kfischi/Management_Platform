 ...
 * Middleware rewrites requests from custom hostnames to this route.
 * We look up the site_id via the `domains` table, then delegate to the
 * same rendering logic as /sites/[siteId]/[[...slug]].
 */
...
async function getSiteIdByDomain(domain: string): Promise<string | null> {
...
export async function generateMetadata({
  ...
  const siteId = await getSiteIdByDomain(decodeURIComponent(domain));
  if (!siteId) return { title: "אתר" };
...
export default async function CustomDomainPage({
  ...
  // Fetch chatbot/whatsapp settings
  ...
  // Build base URL for internal links on custom domain (use root `/`)
  const baseHref = "";
    ...
    <div className="min-h-screen bg-white font-sans" dir="rtl">
      ...
      <nav className="sticky top-0 z-30 border-b border-slate-100 bg-white/95 backdrop-blur-sm shadow-sm">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          <a href="/" className="text-xl font-bold text-slate-900 hover:text-indigo-600 transition-colors">
            {site.name}
          </a>
          {navPages.length > 0 && (
