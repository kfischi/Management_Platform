...
export async function GET(_req: Request, { params }: Params) {
  ...
  const { data, error: dbErr } = await supabase.from("profiles").select("*").eq("id", id).single();
...
export async function PATCH(request: Request, { params }: Params) {
  ...
  const allowed = ["full_name","role","company","phone","website","avatar_url"];
...
export async function DELETE(_req: Request, { params }: Params) {
  ...
  if (error) return error;
  const { id } = await params;
  // Only delete profile metadata — auth user deletion requires service role
